...
export module Visera.Runtime.Render.RHI.Core:Device;
#define VE_MODULE_NAME "RHI.Core:Device"
...
export namespace VE
{
	...
	public:
		...
		void SubmitCommands(EVkQueueFamily _QueueFamily, const VkSubmitInfo _SubmitInfos[], UInt32 _SubmitCounts, VkFence _Fence, UInt32 _QueueIndex = 0) const;

		auto GetHandle() const -> const VkDevice { return Handle; }
		auto GetQueueFamily(EVkQueueFamily Type) const -> const FQueueFamily& { return QueueFamilies[AutoCast(Type)]; }

		Bool IsSupportingExclusiveSwapchain() const { return GetQueueFamily(EVkQueueFamily::Present).Index == GetQueueFamily(EVkQueueFamily::Graphics).Index; }
	...
	private:
		...
		FDevice() noexcept = delete;
		FDevice(SharedPtr<FGPU> _GPU) noexcept;
		~FDevice() noexcept;
	};

	void FDevice::
	SubmitCommands(EVkQueueFamily _QueueFamily, const VkSubmitInfo _SubmitInfos[], UInt32 _SubmitCounts, VkFence _Fence, UInt32 _QueueIndex/* = 0*/) const
	{
		...
		:Host{_GPU}
	{
		for (const auto& GPUCandidate : GPUs)
		{
			...
			QueueFamilies.resize(AutoCast(EVkQueueFamily::All));
			{
				...
				for (UInt32 Index = 0; Index < Properties.size(); ++Index)
				{
					...
					vkGetPhysicalDeviceSurfaceSupportKHR(GPUCandidate.GetHandle(), Index, _Surface, &bPresentSupported);
				...
				{
					QueueFamilies[AutoCast(EVkQueueFamily::Graphics)].Index = IdxA;
					QueueFamilies[AutoCast(EVkQueueFamily::Present)].Index = IdxA;
					for (UInt32 IdxB : TransferQueueFamilies)
					{
						if (IdxB == IdxA) continue;
						QueueFamilies[AutoCast(EVkQueueFamily::Transfer)].Index = IdxB;
						for (UInt32 IdxC : ComputeQueueFamilies)
						{
							if (IdxC == IdxB || IdxC == IdxA) continue;
							QueueFamilies[AutoCast(EVkQueueFamily::Compute)].Index = IdxC;
							Found = True;
						}
						if (Found) break;
					}
			...
			//Extension Supports
			{
				...
				std::transform(GPUCandidate.GetExtensionProperties().begin(),
					           GPUCandidate.GetExtensionProperties().end(),
			...
			//Surface Supports
			{
				...
				vkGetPhysicalDeviceSurfaceFormatsKHR(GPUCandidate.GetHandle(), _Surface, &FormatCount, nullptr);
				if (!FormatCount) continue;

				Array<VkSurfaceFormatKHR> Formats(FormatCount);
				vkGetPhysicalDeviceSurfaceFormatsKHR(GPUCandidate.GetHandle(), _Surface, &FormatCount, Formats.data());

				UInt32 PresentModeCount = 0;
				vkGetPhysicalDeviceSurfacePresentModesKHR(GPUCandidate.GetHandle(), _Surface, &PresentModeCount, nullptr);
				if (!PresentModeCount) continue;

				Array<VkPresentModeKHR> PresentModes(PresentModeCount);
				vkGetPhysicalDeviceSurfacePresentModesKHR(GPUCandidate.GetHandle(), _Surface, &PresentModeCount, PresentModes.data());
				Surface->SetFormats(std::move(Formats));
				Surface->SetPresentModes(std::move(PresentModes));
			}
			
			//Found Suitable Host _GPU_
			{ *_GPU_ = std::move(GPUCandidate); break; }
		}
		if (GVulkan->_GPU_->GetHandle() == VK_NULL_HANDLE)
		{ VE_LOG_FATAL("Failed to find a suitable Physical Device on current computer!"); }

		if (!GVulkan->_GPU_->IsDiscreteGPU())
		{ VE_LOG_WARN("Current _GPU_ is not a discrete _GPU_!"); }

		auto& GraphicsQueueFamily	= QueueFamilies[AutoCast(EVkQueueFamily::Graphics)];
		auto& PresentQueueFamily	= QueueFamilies[AutoCast(EVkQueueFamily::Present)];
		auto& TransferQueueFamily	= QueueFamilies[AutoCast(EVkQueueFamily::Transfer)];
		auto& ComputeQueueFamily	= QueueFamilies[AutoCast(EVkQueueFamily::Compute)];
		...
		VkDeviceCreateInfo DeviceCreateInfo
		{
			...
			.pEnabledFeatures = &GVulkan->_GPU_->GetFeatures()/*m_physical_device_features2.has_value() ? nullptr : &m_physical_device_features*/// (If pNext includes a VkPhysicalDeviceFeatures2, here should be NULL)
		};
		if(vkCreateDevice(
			GVulkan->_GPU_->GetHandle(),
			&DeviceCreateInfo,
